...
namespace RRFFilesManager
{
    ...
    public class PreviewHandlerHost : Control
    {
        ...
        private Guid GetPreviewHandlerGUID(string filename)
        {
            ...
            var extStr = Path.GetExtension(filename);
            RegistryKey ext = Registry.ClassesRoot.OpenSubKey(extStr);
            if (ext != null)
            {
              ...
              if (test == null)
                        test = Registry.ClassesRoot.OpenSubKey("SystemFileAssociations\\" + extStr + "\\ShellEx\\{8895b1c6-b41f-4c1c-a562-0d564250836f}");  
                ...
                if (className != null)
                {
                    ...
                    if (test == null)
                        test = Registry.ClassesRoot.OpenSubKey("SystemFileAssociations\\" + className + "\\ShellEx\\{8895b1c6-b41f-4c1c-a562-0d564250836f}");
                    
                    if (test != null) return new Guid(Convert.ToString(test.GetValue(null)));
                }
                //var perceivedType = Convert.ToString(ext.GetValue("PerceivedType"));
                //if(perceivedType != null)
                //{
                //    test = Registry.ClassesRoot.OpenSubKey("SystemFileAssociations\\" + perceivedType + "\\ShellEx\\{8895b1c6-b41f-4c1c-a562-0d564250836f}");
                //    //if (test == null)
                //        //test = Registry.ClassesRoot.OpenSubKey("SystemFileAssociations\\" + perceivedType + "\\ShellEx\\{e357fccd-a995-4576-b01f-234630154e96}");
                //    if (test == null)
                //        test = Registry.ClassesRoot.OpenSubKey("SystemFileAssociations\\" + perceivedType + "\\shellex\\{8895b1c6-b41f-4c1c-a562-0d564250836f}");
                //    //if (test == null)
                //        //test = Registry.ClassesRoot.OpenSubKey("SystemFileAssociations\\" + perceivedType + "\\shellex\\{e357fccd-a995-4576-b01f-234630154e96}");
        ...
        public bool Open(string filename)
        {
            ...
            if (guid != Guid.Empty)
            {
                try
                {
                    ...
                    //if (mCurrentPreviewHandler is IThumbnailProvider)
                    //{
                    //    // bind the preview handler to the control's bounds and preview the content
                    //    Rectangle r = ClientRectangle;
                    //    IntPtr phbmp;
                    //    WTS_ALPHATYPE pdwAlpha;
                    //    var handler = ((IThumbnailProvider)mCurrentPreviewHandler).GetThumbnail((uint)r.Width, out phbmp, out pdwAlpha);
